...
```{r}
point_estimate(metropolis.cw.mcmc)
```

```{r}
# 95% credible regions
HPDinterval(metropolis.cw.mcmc)
```

### 3.e

```{r}
metropolis.cw.results.t0 <- metropolis.cw(
  theta.init = c(5.5, 0.98, 0.01),
  sd.proposals = sd.proposals,
  t = c(0),
  experiment = df$Exp1,
...
# goal is to reach an acceptance rate of 0.4
metropolis.cw.results.t0$accept.rate
```

```{r}
max.int <- as.integer64(.Machine$integer.max) + 1L

metropolis.cw.results.tinf <- metropolis.cw(
  theta.init = c(5.5, 0.98, 0.01),
  sd.proposals = sd.proposals,
  t = c(max.int),
  experiment = df$Exp1,
...
# goal is to reach an acceptance rate of 0.4
metropolis.cw.results.tinf$accept.rate
```
...
```{r}
point_estimate(metropolis.vw.mcmc)
```

```{r}
# 95% credible regions
HPDinterval(metropolis.vw.mcmc)
```
